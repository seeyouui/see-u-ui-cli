 ...
 * - CSS 注释格式
 */
export function conditionalCompilationPlugin(platform: string = 'H5'): Plugin {
  return vitePluginUniConditionalCompile({
...
export default function vitePluginUniConditionalCompile(options: ConditionalCompileOptions = {}): Plugin {
  ...
  return {
    ...
    transform(code: string, id: string): TransformResult | undefined {
      ...
      // 检查是否包含条件编译指令
      if (!code.includes('#ifdef') && !code.includes('#ifndef') && !code.includes('#endif')) {
        return undefined
      }
      ...
      try {
        ...
        transformed = transformed.replace(htmlConditionalPattern, (_match, type, envExp, content) => {
          return processCondition(type, envExp, content, platform, isTest)
        })
...
function evaluateCondition(condition: string, platform: string): boolean {
  ...
  const platformMap: Record<string, boolean> = {
    ...
    APP: platform.toUpperCase() === 'APP' || platform.toUpperCase() === 'APP-PLUS',
    'APP-PLUS': platform.toUpperCase() === 'APP' || platform.toUpperCase() === 'APP-PLUS',
    MP: platform.toUpperCase().startsWith('MP-'),
    'MP-WEIXIN': platform.toUpperCase() === 'MP-WEIXIN',
    'MP-ALIPAY': platform.toUpperCase() === 'MP-ALIPAY',
    'MP-BAIDU': platform.toUpperCase() === 'MP-BAIDU',
    'MP-TOUTIAO': platform.toUpperCase() === 'MP-TOUTIAO',
    'MP-QQ': platform.toUpperCase() === 'MP-QQ'
  }
  ...
  // 检查平台映射
  if (platformMap[targetPlatform] === true) {
    matched = true
  }
  ...
  // 处理 MP 这种通用标识
  if (targetPlatform === 'MP' && currentPlatform.startsWith('MP-')) {
    matched = true
  }
